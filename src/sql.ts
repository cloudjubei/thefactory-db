...
WHERE project_id = ANY($1::text[])
  AND (
    $2::text = ''
    OR lower(src) LIKE '%' || lower($2) || '%' ESCAPE '\\'
    OR lower(name) LIKE '%' || lower($2) || '%' ESCAPE '\\'
  )
  AND (
    $3::text IS NULL
    OR lower(src) LIKE lower($3) || '%' ESCAPE '\\'
  )
ORDER BY src ASC
...
// ----------------------------------------------------------
// Ranked keyword + literal searches (paths only)
// - Uses DB functions defined in @docs/SEARCH_IMPROVED.md
//   keyword_search_documents(query_text, match_count, filter_jsonb, include_name_and_src)
//   literal_search_documents(query_text, match_count, filter_jsonb, include_name_and_src, case_sensitive)
// ----------------------------------------------------------
const keywordSearchDocumentsForPaths = `
SELECT src
FROM keyword_search_documents(
  $2::text,
  COALESCE($5::int, 20),
  jsonb_build_object('projectIds', to_jsonb($1::text[])),
...
ORDER BY rrf_score DESC, keyword_score DESC, src ASC;
`

const literalSearchDocumentsForPaths = `
SELECT src
FROM literal_search_documents(
  $2::text,
  COALESCE($6::int, 20),
  jsonb_build_object('projectIds', to_jsonb($1::text[])),
  COALESCE($4::boolean, true),
...
export const SQL = {
  ...
  upsertDocument,
  insertDocument,
  keywordSearchDocumentsForPaths,
  literalSearchDocumentsForPaths,
  updateDocument,
