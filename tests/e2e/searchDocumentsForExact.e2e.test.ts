...
;(RUN && DATABASE_URL ? describe : describe.skip)('E2E: db.searchDocumentsForExact', () => {
  ...
  beforeAll(async () => {
    ...
    await db.addDocument({
      ...
      name: 'D',
      src: 'docs/d.txt',
      content: 'Alpha (uppercase) only once',
    })
    await db.addDocument({
      projectId,
      type: 'txt',
  ...
  it('throws if args.needles is not string|string[]', async () => {
    // @ts-expect-error
    await expect(
      db.searchDocumentsForExact({ projectIds: [projectId], needles: 1 }),
  ...
  it('throws if args.matchMode is invalid', async () => {
    // @ts-expect-error
    await expect(
      db.searchDocumentsForExact({ projectIds: [projectId], needles: ['a'], matchMode: 'nope' }),
  ...
  it('throws if args.caseSensitive is not boolean', async () => {
    // @ts-expect-error
    await expect(
      db.searchDocumentsForExact({ projectIds: [projectId], needles: ['a'], caseSensitive: 'yes' }),
  ...
  it('default matchMode is any', async () => {
    const any = await db.searchDocumentsForExact({
      projectIds: [projectId],
      needles: ['alpha', 'gamma'],
      caseSensitive: true,
      limit: 10,
  ...
  it('matchMode=all requires all needles', async () => {
    const all = await db.searchDocumentsForExact({
      projectIds: [projectId],
      needles: ['alpha', 'gamma'],
      matchMode: 'all',
      caseSensitive: true,
