...
;(RUN && DATABASE_URL ? describe : describe.skip)(
  'E2E: Updates and FTS regeneration (real DB)',
  () => {
    ...
    it('entities: updates should regenerate updatedAt and FTS on content changes', async () => {
      ...
      // FTS should not match "beta" initially
      r = await db
        .raw()
        .query(
          "SELECT fts @@ websearch_to_tsquery('english', $1) AS match FROM entities WHERE id = $2",
      ...
      const entUpd = await db.updateEntity(ent.id, {
        content: { title: 'Beta token', desc: 'second' },
      })
