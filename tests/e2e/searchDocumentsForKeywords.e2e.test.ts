...
;(RUN && DATABASE_URL ? describe : describe.skip)('E2E: db.searchDocumentsForKeywords', () => {
  ...
  it('throws if args.keywords is not string|string[]', async () => {
    // @ts-expect-error
    await expect(
      db.searchDocumentsForKeywords({ projectIds: [projectId], keywords: 1 }),
  ...
  it('tokenizes when keywords is string: split on comma/semicolon only', async () => {
    ...
    const res2 = await db.searchDocumentsForKeywords({
      projectIds: [projectId],
      keywords: 'alpha, beta',
      limit: 10,
    })
  ...
  it('default matchMode is any', async () => {
    const any = await db.searchDocumentsForKeywords({
      projectIds: [projectId],
  ...
  it('matchMode=all requires all tokens', async () => {
    const all = await db.searchDocumentsForKeywords({
      projectIds: [projectId],
      keywords: ['alpha', 'gamma'],
      matchMode: 'all',
      limit: 10,
